...
import { map, catchError, startWith, publishReplay } from 'rxjs/operators';
import { Router } from '@angular/router';
import { LoginSuccess, AuthEvent, LoginFail } from './auth.events';
...
export class AuthMachine {
  authMachineOptions: Partial<MachineOptions<AuthContext, AuthEvent>> = {
    services: {
      requestLogin: (_, event) =>
        this.authService.login({ email: event.username, password: event.password }).pipe(
  ...
  private _authMachine = Machine<AuthContext, AuthSchema, AuthEvent>(authMachineConfig).withConfig(this.authMachineOptions);
